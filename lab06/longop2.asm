...
; Shift right N bits and fills the free space with the given string of bits. Expects the result to be clean(0s).
; 1st arg = integer length(in 4-byte chunks) [EBP + 24]
; 2nd arg = integer address [EBP + 20]
; 3rd arg = result address [EBP + 16]
; 4th arg = N (amount of bits to shift) [EBP + 12]
...
ShrLongN proc
  ...
  mov eax, edx
  shl eax, 2 ; *4
  add esi, eax ; manually shift the source, so that we write from the highest chunks into shifted chunks
@shr_long_move_integer_loop:
  mov ebx, dword ptr[esi + ecx * 4 - 4]  
  mov dword ptr[edi + ecx * 4 - 4], ebx

  dec ecx  
  jnz @shr_long_move_integer_loop


  ; FILL THE INTEGER WITH THE CHUNKS(whole) FROM THE STRING
  ; if chunks == 0 then skip this part
  cmp edx, 0
  je @shr_long_move_string_loop_done
  ...
  mov eax, ecx
  shl eax, 2 ; *4
  add edi, eax ; manually shift the destination, so that we fill starting from the highest chunks

  mov ecx, edx ; EDX contains amount of chunks that were shifted
@shr_long_move_string_loop:
  mov ebx, dword ptr[esi + ecx * 4 - 4]
  mov dword ptr[edi + ecx * 4 - 4], ebx

  dec ecx
  jnz @shr_long_move_string_loop
@shr_long_move_string_loop_done:


  ; SHIFT THE LEFTMOST(LOW) CHUNK OF THE INTEGER
  ;mov ebx, dword ptr[ebp + 24] ; integer length == result length
  mov esi, dword ptr[ebp + 16] ; result address
  mov ecx, dword ptr[ebp + 12] ; N
  and ecx, 31 ; %32 => bits in one chunk to shift. (00011111b)
  shr dword ptr[esi], cl ; shift the leftmost chunk  
  ...
  ; if ecx == 0 => skip this part
  cmp ecx, 0
  je @shr_long_shift_chunks_loop_done
  
  mov esi, dword ptr[ebp + 16] ; result address
  ;mov ecx, dword ptr[ebp + 24] ; integer length == result length
  mov ecx, 1 ; skip the 0th one(low) 
  ;dec ecx
@shr_long_shift_chunks_loop:
  mov edx, 32
  sub edx, ebx ; the chunk has to be shifted in the opposite direction in order to apply the mask for the previous(lower) chunk
  mov eax, dword ptr[esi + ecx * 4] ; the chunk we're working with (at first iteration it will be the first one(right after 0th))  
  push ecx  
  mov ecx, edx ; second operand for shift has to be CL
  shl eax, cl ; shift current chunk in the opposite direction
  pop ecx
  mov edx, dword ptr[esi + ecx * 4 - 4] ; previous(lower) chunk is -4 relative to the current chunk
  or edx, eax ; apply the mask stored in eax === put there the bits that will fall out because of shifting the current chunk
  mov dword ptr[esi + ecx * 4 - 4], edx ; put the previous(lower) chunk back into memory
  ; Now safely shift(because we've processed the bits that fall out) the current chunk(in memory)  
  push ecx
  mov eax, ecx ; used in the address for dword ptr two lines down
  mov ecx, ebx ; second operand for shift has to be CL
  shr dword ptr[esi + eax * 4], cl ; MAIN OPERATION. Amount of bits to shift is stored in EBX  
  pop ecx  

  inc ecx
  cmp ecx, dword ptr[ebp + 24]
  jl @shr_long_shift_chunks_loop
@shr_long_shift_chunks_loop_done:


  ; FILL WITH BITS FROM THE STRING THE LAST(HIGH) CHUNK IN THE INTEGER
  ; if ebx == 0 (so only whole chunks needed to be shifted) => skip this part
  cmp ebx, 0
  je @shr_long_fill_last_chunk_done

  mov esi, dword ptr[ebp + 8] ; string address    
  mov edi, dword ptr[ebp + 16] ; result address
  ; We know for sure that the bits we need are in the last(highest) chunk(both source and destination), so shift the memory  
  mov eax, dword ptr[ebp + 24] ; integer length
  dec eax ; because we count from 0
  shl eax, 2 ; *4 => bytes  
  add edi, eax ; now points to the last(highest) chunk

  mov eax, dword ptr[ebp + 12] ; N
  dec eax ; because we count from 0
  shr eax, 3 ; /32 * 4 => integer shift (full chunks to shift) in bytes
  add esi, eax  
  ; We also know that the last(highest) chunk has already been shifted in the loop above
  ; The bits in the string that we need are in the lowest part of the chunk
  mov eax, dword ptr[esi] ; the chunk from the string that wee need
  mov ecx, 32
  sub ecx, ebx ; same routine as before
  shl eax, cl ; We shift in the opposite direction
  mov edx, dword ptr[edi] ; the chunk in the integer that we are modifying
  or edx, eax ; apply the mask === put in the bits
  mov dword ptr[edi], edx ; put back into memory
@shr_long_fill_last_chunk_done:
...
; Calculates the amount of zeros in a long integer
; 1st arg = integer length(in 4-byte chunks) [EBP + 16]
; 2nd arg = integer address [EBP + 12]
; 3rd arg = result address [EBP + 8]
AmountOfZerosLong proc
  push ebp
  mov ebp, esp

  xor ebx, ebx ; =0. Counter(result)

  mov ecx, dword ptr[ebp + 16] ; integer length(in 4-byte chunks)
  mov esi, dword ptr[ebp + 12] ; integer address 
  mov edi, dword ptr[ebp + 8] ; result address 

@amount_of_zeros_chunks_loop: ; OUTTER LOOP START
  mov eax, dword ptr[esi + 4 * ecx - 4]
  mov edx, 32 ; bit in one chunk 

@amount_of_zeros_bits_loop: ; INNER LOOP START
  shr eax, 1
  jc @amount_of_zeros_bit_0
  inc ebx
  ;jmp @amount_of_zeros_bit_done
;@amount_of_zeros_bit_0:  
@amount_of_zeros_bit_0:
  

;@amount_of_zeros_bit_done:
  dec edx
  jnz @amount_of_zeros_bits_loop ; INNER LOOP END

  dec ecx
  jnz @amount_of_zeros_chunks_loop ; OUTTER LOOP END

  ; Write the result(ebx)
  mov dword ptr[edi], ebx

  mov esp, ebp
  pop ebp
  ret 12  
AmountOfZerosLong endp
...
@amount_of_zeros_bits_loop: ; INNER LOOP START
  shr eax, 1
  jnc @amount_of_zeros_bit_1
  inc ebx
  ;jmp @amount_of_zeros_bit_done
;@amount_of_zeros_bit_0:  
@amount_of_zeros_bit_1:
...
@mul_long_loop:   
  ...
  shl eax, 2 ; max possible operand length(in buffer)
  push eax
